...
export const OrdersPage: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return orders.filter(o => {
      ...
      if (search && !`#${o.id} gia-${o.id} ${o.customer.name} ${o.customer.doc}`.toLowerCase().includes(search.toLowerCase())) return false;
      return true;
    });
  ...
  const handleDeleteOrder = async (orderId: number, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!window.confirm(`¿Estás seguro de eliminar permanentemente el pedido GIA-${orderId}?`)) {
      return;
    }
    try {
      await deleteOrder(orderId);
      toast.success(`Pedido GIA-${orderId} eliminado`);
      ...
      <div className="bg-white rounded-2xl p-3 border border-[#364266]/10 shadow-sm flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 font-sans">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Buscar por # pedido (ej. GIA-1003), cliente o documento..."
              ...
              >
                <div>
                  ...
                  <div className="flex items-start justify-between gap-2 pb-2 border-b border-gray-100">
                    <div>
                      <span className="font-bold text-sm text-[#364266] font-sans">GIA-{order.id}</span>
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.map(order => (
                  <tr key={order.id} className="hover:bg-[#FAF8EA]/50 transition-colors">
                    <td className="py-3 px-4 font-bold text-[#364266] font-sans">GIA-{order.id}</td>
